...
class Solution{
    Node* buildTree(int pre[],int &i, int bound, int n)
    {
        if(i == n || pre[i] > bound) return NULL;
        
        Node* root=newNode(pre[i++]);
        root->left=buildTree(pre,i,root->data,n);
        root->right=buildTree(pre,i,bound,n);
...
public:
    ...
    Node* post_order(int pre[], int size)
    {
        //code here
        int i=0;
        return buildTree(pre,i,INT_MAX,size);
    }
};
